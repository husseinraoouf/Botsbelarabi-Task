    ...
    methods.getWeatherInTime = async (lat, long, time, withTime, unit = 'ca', timezone) => {
        ...
        if (withTime) {
            result = {
                ...
                'icon': helper.getIcon(data.currently.icon),
                'time': helper.convertToTimezone(data.currently.time, timezone),
                'chanceOfRain': (data.currently.precipProbability * 100).toFixed(2),
            }
        } else {
            result = {
                ...
                'chanceOfRain': (data.daily.data[0].precipProbability * 100).toFixed(2),
            }
        }
